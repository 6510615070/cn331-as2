...
class RegisterSubjectViewTest(TestCase):
    ...
    def test_successful_registration(self):
        ...
        response = self.client.post(reverse('register_subject', args=[self.subject.id]))
        self.assertRedirects(response, reverse('subject_list'))
        
        # Confirm that the UserSubject instance was created
        self.assertTrue(UserSubject.objects.filter(user=self.user, subject=self.subject).exists())
    ...
    def test_registration_restricted_when_full(self):
        ...
        registration_count = UserSubject.objects.filter(user=self.user, subject=self.subject).count()
...
class UnregisterSubjectViewTest(TestCase):
    ...
    def test_successful_unregistration(self):
        ...
        response = self.client.post(reverse('unregister_subject', args=[self.subject.id]))
        self.assertRedirects(response, reverse('subject_list'))
        
        # Confirm that the UserSubject instance is deleted
        self.assertFalse(UserSubject.objects.filter(user=self.user, subject=self.subject).exists())
